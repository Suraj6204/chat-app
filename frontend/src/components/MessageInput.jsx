  ...
  return (
    <div className="p-4 w-full bg-base-100 relative">
      ...
      {menuOptions && (
        <div
          ref={menuRef}
          className="absolute bottom-full left-4 mb-2 w-56 bg-base-200 border border-base-300 rounded-2xl shadow-2xl py-2 z-50"
        >
          <MenuOptions
            icon={FileText}
            label="Document"
            ...
            <div className="m-2 p-3 bg-base-300/60 rounded-xl border-l-4 border-primary flex items-center justify-between animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-semibold text-primary">
                  Replying to{" "}
                  {replyPreviewMessage.senderId === authUser._id
                    ? "yourself"
            ...
            <textarea
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && !e.shiftKey) {
                  e.preventDefault(); // Default line break stop karo
                  if (text.trim() || imagePreview) {
                    handleSendMessage(e); // Form submit hit karo
                  }
                }
              }}
            />
